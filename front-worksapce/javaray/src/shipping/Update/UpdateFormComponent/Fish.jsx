import {
  DeleteFishs,
  DeleteImage,
  FishP,
  FishsExDiv,
} from "../ShippingUpdateCss";

const ShippingFish = ({ fish, setFish, fishs }) => {
  const deleteFish = (e, name) => {
    const newFish = fish.filter((fish) => {
      return fish !== e;
    });
    setFish(newFish, name);
  };
  return (
    <FishsExDiv key={fishs.fishNo}>
      <DeleteFishs onClick={() => deleteFish(fishs, "fishs")}>
        <DeleteImage src="data:image/png;base64," />
      </DeleteFishs>
      <FishP>{fishs.fishName}</FishP>
    </FishsExDiv>
  );
};

export default ShippingFish;
