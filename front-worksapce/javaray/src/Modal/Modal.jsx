...
const Modal = (props) => {
  ...
  const address = props.port;
  return (
    <>
      <ModalBackground>
        <ModalWrap>
          <CloseDiv onClick={() => clickModal(false)}>
          ...
          {id === "deleteManager" || id === "ReportUser" ? (
            <ReportForm kind={id} shippingNo={props.shippingNo} data={data} setData={setData}clickModal={clickModal}/>
          ) : (
            <></>
          )}
